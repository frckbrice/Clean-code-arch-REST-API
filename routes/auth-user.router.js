...
router
  .route('/auth/register')
  .post(async (req, res) =>
    makeResponseCallback(userControllerHandlers.registerUserControllerHandler)(req, res)
  );

router
  .route('/auth/login')
  .post(loginLimiter, async (req, res) =>
    makeResponseCallback(userControllerHandlers.loginUserControllerHandler)(req, res)
  );
...
router
  ...
  .post(async (req, res) =>
    makeResponseCallback(userControllerHandlers.forgotPasswordControllerHandler)(req, res)
  );
...
router // TODO: implement reset password simulated. update this with the correct route for reset password in line 32 below
  ...
  .put(async (req, res) =>
    makeResponseCallback(userControllerHandlers.resetPasswordControllerHandler)(req, res)
  );
...
router
  .route('/')
  .get(authVerifyJwt, isAdmin, async (req, res) =>
    makeResponseCallback(userControllerHandlers.findAllUsersControllerHandler)(req, res)
  );

router
  .route('/logout')
  .post(async (req, res) =>
    makeResponseCallback(userControllerHandlers.logoutUserControllerHandler)(req, res)
  );

router
  .route('/refresh')
  .get(async (req, res) =>
    makeResponseCallback(userControllerHandlers.refreshTokenUserControllerHandler)(req, res)
  );

//authVerifyJwt, isAdmin,
router
  .route('/:userId')
  .get(authVerifyJwt, isAdmin, isBlocked, async (req, res) =>
    makeResponseCallback(userControllerHandlers.findOneUserControllerHandler)(req, res)
  );

// Remove or comment out routes that reference undefined controller handlers
// router
//   .route('/:userId')
