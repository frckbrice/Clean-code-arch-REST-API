...
function validateDescription({ description, InvalidPropertyError }) {
  if (description.length < 50) {
    throw new InvalidPropertyError(`A product's title must be at least 20 characters long.`);
  }
...
function validateCategory(category, InvalidPropertyError) {
  if (!category || !category.ref || !category.id) {
    throw new InvalidPropertyError(`A product must have a valid category.`);
  }
...
function validateNumber(quantity, InvalidPropertyError) {
  if (quantity < 0) {
    throw new InvalidPropertyError(`A product's quantity must be greater than or equal to 0.`);
  }
  return quantity;
}
...
const calculateAverageRating = (totalRatings, resultingProductData) => {
  if (!totalRatings || !totalRatings.length) return 0;
  return (
    totalRatings.reduce((acc, curr, idx) => acc + curr * (idx + 1), 0) /
    resultingProductData.totalReviews
  );
};
...
const basicProductValidation = ({ productData, errorHandlers }) => {
  ...
  if (!productData.title) {
    ...
    resultingProductData.title = validateTitle({ title: productData.title, InvalidPropertyError });

  if (!productData.descripton) {
    errors.push(`Product descripton is required`);
  } else
    resultingProductData.description = validateDescription({
  ...
  if (productData.salePrice) {
    resultingProductData.salePrice =
      validateNumber(productData.salePrice) || resultingProductData.price;
  }
