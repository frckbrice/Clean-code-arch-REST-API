...
const validateCoverImage = ({ cover_image, InvalidPropertyError }) => {
  ...
  if (ext !== 'png' && ext !== 'jpeg' && ext !== 'jpg') {
    throw new InvalidPropertyError(`Invalid cover image.`);
  }
  return cover_image;
};
...
const validateTags = (tags, InvalidPropertyError) => {
  if (!tags || !tags.length) {
    throw new InvalidPropertyError(`Tags are required.`);
  }
  return tags ?? [];
};
...
const blogPostValidation = ({ blogPostData, errorHandlers }) => {
  // blog post props
  const { title, content, author, tags, image_urls, cover_image } = blogPostData;
  ...
  if (!content) {
    ...
    resultingBlogPostData.content = validateDescription({
      description: content,
      InvalidPropertyError,
    });

  if (!cover_image) errors.push(`Blog post cover image is required`);
  else
    resultingBlogPostData.cover_image = validateCoverImage({ cover_image, InvalidPropertyError });
  if (!author) {
